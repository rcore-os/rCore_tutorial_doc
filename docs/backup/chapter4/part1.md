...
这个扫描结果描述了所有外设的信息，当中也包括我们的内存。不过为了简单起见，我们并不打算自己去解析这个结果。因为我们知道，Qemu 规定的物理内存开头的物理地址为 $$0\text{x}80000000$$ 。而在 Qemu 中，可以使用 ``-m`` 指定 RAM 的大小，默认是 $$128\text{MiB}$$ 。因此，默认的物理内存地址范围就是 $$[0\text{x}80000000,0\text{x}88000000)$$ 。我们直接将物理内存结束地址硬编码到内核中：
...
* 物理地址空间 $$[0\text{x}80000000,0\text{x}80200000)$$ 被 OpenSBI 占用；
* 物理地址空间$$[0\text{x}80200000,\text{KernelEnd})$$ 被内核各代码与数据段占用；
* 其实设备树扫描结果 DTB 还占用了一部分物理内存，不过由于我们不打算使用它，所以可以将它所占用的空间用来存别的东西。

于是，我们可以用来存别的东西的物理内存的物理地址范围是：$$[\text{KernelEnd}, 0\text{x}88000000)$$ 。这里的 $$\text{KernelEnd}$$ 为内核代码结尾的物理地址。在 ``linker64.ld`` 中定义的 ``end`` 符号为内核代码结尾的虚拟地址，我们需要通过偏移量来将其转化为物理地址。
...
所以我们的方法是使用 ``Mutex<T>`` 给这段数据加一把锁，一个线程试图通过 ``.lock()`` 打开锁来获取内部数据的可变引用，如果钥匙被别的线程所占用，那么这个线程就会一直卡在这里；直到那个占用了钥匙的线程对内部数据的访问结束，锁被释放，将钥匙交还出来，被卡住的那个线程拿到了钥匙，就可打开锁获取内部引用，访问内部数据。
